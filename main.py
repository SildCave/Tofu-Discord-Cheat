...
if __name__ == '__main__':
    ...
    async def handle_drop(message):
        if (message.author.id == tofu_user_id) and ' is summoning 2 cards!' in message.content:
            ...
            for worker in workers:
                if int(worker.id) == int(message.content.replace(' is summoning 2 cards!', '').replace('>', '').replace('<@', '')):
                    ...
                    print(f"GRAB | {worker.name}")
                    break


        if (message.author.id == tofu_user_id) and ' grabbed a **Fusion Token**!<a:token:973893149770522674>' in message.content:
            for worker in workers:
                if int(worker.id) == int(message.content.replace(' grabbed a **Fusion Token**!<a:token:973893149770522674>', '').replace('>', '').replace('<@', '')):
                    ...
                    print(f"FUSION OPEN | {worker.name}")

                    break

        try:
            if (message.author.id == tofu_user_id) and ('receive a random card from' in message.embeds[0].description):
                ...
                for worker in workers:
                    if int(worker.id) == fusion_worker_id:
                        ...
                        print(f"FUSION ACCEPT | {worker.name}")
